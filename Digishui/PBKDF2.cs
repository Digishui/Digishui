/*
 * Derived from https://github.com/shawnmclean/SimpleCrypto.net
 *
...
namespace Digishui
{
  ...
  public class PBKDF2
  {
    ...
    public string Compute()
    {
      ...
      if (string.IsNullOrEmpty(Salt)) { GenerateSalt(); }
    ...
    public string GenerateSalt()
    {
      ...
      byte[] generatedSalt = new byte[SaltSize];

      randomNumberGenerator.GetBytes(generatedSalt);

      //assign the generated salt in the format of {iterations}.{salt}
      Salt = string.Format("{0}.{1}", HashIterations, Convert.ToBase64String(generatedSalt));
    ...
    private string CalculateHash(int iteration)
    {
      ...
      byte[] derivedKey = Rfc2898DeriveBytes.Pbkdf2(PlainText, saltBytes, iteration, HashAlgorithmName.SHA1, 64);
    ...
    private void ExpandSalt()
    {
      try
      {
        ...
        HashIterations = int.Parse(Salt[..i], System.Globalization.NumberStyles.Number);
    ...
    public static bool Compare(string passwordHash1, string passwordHash2)
    {
      ...
      for (int i = 0; i < min_length; i++) { result |= passwordHash1[i] ^ passwordHash2[i]; }
